...
generate_style_old.default_json = {
...
generate_style_old.dialog_json = {
  guidance = {
  ...
  documentation = "https://www.pixellab.ai/docs/tools/style_old"
}

generate_style_old.current_json = createJson.shallowcopy(generate_style_old.default_json)

function generate_style_old.prepare_image(model, create_new_frames)
...
function generate_style_old.check_size()
...
function generate_style_old.generate(model, default_current, dlg_title, dlg_type)
  ...
  elseif model.current_json["use_selection"] == false and generate_style_old.check_size() and model.current_json["max_size"][1] == 64 and model.current_json["max_size"][2] == 64 then
  ...
  local cels = generate_style_old.prepare_image(model, true)
...
function generate_style_old.onClose(model)
...
function generate_style_old.update_selection(ev)
  if ev.fromUndo then
    return
  end
  if generate_style_old.current_json["use_selection"] and (previous_origin == nil or previous_bounds == nil or previous_origin.x ~= app.activeSprite.selection.origin.x or previous_origin.y ~= app.activeSprite.selection.origin.y or
    ...
    if app.activeSprite.selection.bounds.width ~= 0 and app.activeSprite.selection.bounds.height ~= 0 then
      if generate_style_old.current_json["max_size"][1] == 64 then
        ...
        local maxArea = generate_style_old.current_json["max_size"][1] * generate_style_old.current_json["max_size"][2]
...
function generate_style_old.openDialog(model)
  generate_style_old.current_json = model.current_json
  if app.activeSprite == nil then
  ...
  if app.activeSprite ~= nil then
    if listenerCode == nil then
      ...
      listenerCode = event_sprite.events:on('change', generate_style_old.update_selection)
    end
  end
