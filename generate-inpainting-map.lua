...
generate_inpainting_map.default_json = {
  description = "pond",
...
generate_inpainting_map.dialog_json = {
  guidance = {
    view = { "high top-down", "side" },
    text_guidance_scale = { 15, 90 },
    inpainting_guidance_scale = { 10, 60 },
  },
  documentation = "https://www.pixellab.ai/docs/tools/map-tiles-new"
}

generate_inpainting_map.current_json = createJson.deepcopy(generate_inpainting_map.default_json)

function generate_inpainting_map.prepare_image(model, create_new_frames)
...
function generate_inpainting_map.generate(model, default_current, dlg_title, dlg_type)
  ...
  copy_data["view_direction"] = true
  copy_data["transparent_background"] = false
  ...
  local cels = generate_inpainting_map.prepare_image(model, true)
...
function generate_inpainting_map.onClose(model)
...
function generate_inpainting_map.update_selection(ev)
  if ev.fromUndo then
    return
  end
  ...
  if generate_inpainting_map.current_json["use_selection"] and (previous_origin == nil or previous_bounds == nil or previous_origin.x ~= app.activeSprite.selection.origin.x or previous_origin.y ~= app.activeSprite.selection.origin.y or
...
function generate_inpainting_map.openDialog(model)
  generate_inpainting_map.current_json = model.current_json
  if app.activeSprite == nil then
  ...
  if app.activeSprite ~= nil then
    if listenerCode == nil then
      ...
      listenerCode = event_sprite.events:on('change', generate_inpainting_map.update_selection)
    end
...
return generate_inpainting_map
