...
generate_character.default_json = {
  description = "wizard",
...
generate_character.dialog_json = {
  guidance = {
  ...
  documentation = "https://www.pixellab.ai/docs/tools/create-character"
}

generate_character.current_json = createJson.shallowcopy(generate_character.default_json)

function generate_character.prepare_image(model, create_new_frames)
...
function generate_character.check_size()
...
function generate_character.generate(model, default_current, dlg_title, dlg_type)
  ...
  if app.activeSprite == nil then
    ...
    -- elseif generate_character.check_size() then
    --   return app.alert("Canvas must be size 128x128, 64x64, 32x32 or 16x16")
  end
  -- elseif model.current_json["use_selection"] == false and generate_character.check_size() and model.current_json["max_size"][1] == 64 and model.current_json["max_size"][2] == 64 then
  ...
  local cels = generate_character.prepare_image(model, true)
...
function generate_character.onClose(model)
...
function generate_character.update_selection(ev)
  if ev.fromUndo then
    return
  end
  if generate_character.current_json["use_selection"] and (previous_origin == nil or previous_bounds == nil or previous_origin.x ~= app.activeSprite.selection.origin.x or previous_origin.y ~= app.activeSprite.selection.origin.y or
    ...
    if app.activeSprite.selection.bounds.width ~= 0 and app.activeSprite.selection.bounds.height ~= 0 then
      if generate_character.current_json["max_size"][1] == 64 then
        ...
        local maxArea = generate_character.current_json["max_size"][1] * generate_character.current_json["max_size"][2]
...
function generate_character.openDialog(model)
  generate_character.current_json = model.current_json
  if app.activeSprite == nil then
  ...
  --     listenerCode = event_sprite.events:on('change', generate_character.update_selection)
  --   end
  -- end
